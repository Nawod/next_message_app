...
export const TextBox = () => {
	...
	const { replyId, selectedMessage } = useSelector(
		(state: RootState) => state.messageStates
	);
		...
		<div className="flex gap-4 rounded-lg p-4 bg-white">
			...
			>
				<textarea
					...
					className={`resize-none border-2 rounded-md w-full h-24 py-4 px-7 outline-none ${
				...
				<input
					type="submit"
					className="text-white bg-purple-700 hover:bg-purple-500 cursor-pointer font-semibold h-fit rounded-md py-3 px-7 uppercase"
					value="Send"
				/>
			</form>
		</div>
